...
class Pagination extends Component {
    ...
    render() {
        ...
        for (let i = 1; i <= this.props.totalPages; i++) {
            const className =
                parseInt(this.props.activePage) === i ? "active" : "";
            const page = (
                <button className={className} key={i} onClick={this.getPerPage}>
                    {i}
                </button>
            );
            pagesList.push(page);
        ...
        return (
            <div className="pagination">
                ...
                <div className="pagination-info">
                    showing item {(this.props.activePage - 1) * 10 + 1} -{" "}
                    {(this.props.activePage - 1) * 10 + this.props.items.length}{" "}
                    of {this.props.count} items
                </div>
            </div>
        );
    }
